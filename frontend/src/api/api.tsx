...
try { return localStorage.getItem('sb_token') || undefined } catch { return undefined }
}
...
const resp = await request<AuthResponse>('/auth/signup', { method: 'POST', body: JSON.stringify(payload) })
...
const resp = await request<AuthResponse>('/auth/login', { method: 'POST', body: JSON.stringify({ email, password: payload.password }) })
setAuth(resp.token, resp.user)
return resp
}


export async function me() { return request<AuthUser>('/auth/me', { method: 'GET' }) }
export function logout() { setAuth(undefined, undefined) }
