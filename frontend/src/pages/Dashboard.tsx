    ...
    const gameModes = [
        {
            id: 'classic',
            title: 'CLASSIC MODE',
            description: 'Identify tracks from audio snippets. Test your music knowledge with timed challenges.',
        },
        {
            id: 'inference',
    ...
    return (
        <>
            ...
            <div className="min-h-screen max-h-screen overflow-hidden items-center justify-center font-montserrat p-4 relative">
                ...
                {/* Navigation Bar */}
                <nav className="relative z-50 flex items-center justify-between p-6 bg-darkblue rounded-xl" style={{ borderBottom: `1px solid rgba(255,255,255,0.10)`, backdropFilter: 'blur(6px)' }}>
                    <div className="flex items-center space-x-8">
                        <h1 className="text-2xl font-exo font-bold text-teal tracking-widest" style={{ textShadow: '0 0 20px rgba(15, 193, 233, 0.5)' }}>
                            SOUNDBYTE
                        </h1>
                    </div>
                ...
                <div className="flex h-screen">
                                            ...
                                            >
                                                ...
                                                >
                                                    ...
                                                    <motion.img
                                                        ...
                                                        transition={{ repeat: Infinity, ease: 'linear', duration: selectedMode === mode.id ? 5 : 0 }}
                                                    />
                                                </div>
                    ...
                    <div className="w-1/3 p-10 space-y-6">
                        {/* Recent Activity */}
                        <div className="bg-darkblue/80 backdrop-blur-sm rounded-2xl p-4" style={{ border: '1px solid rgba(255,255,255,0.10)' }}>
                            ...
                            <div className="space-y-3 max-h-64 overflow-y-auto scrollbar-thin scrollbar-track-transparent scrollbar-thumb-teal">
                                {recentActivity.map((activity) => (
                                    <div key={activity.id} className="text-sm text-grayblue">
                                        <p>{activity.action}</p>
                                        <p className="text-xs text-grayblue/70">Score: {activity.score} • {activity.time}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="bg-darkblue/80 backdrop-blur-sm rounded-2xl p-4" style={{ border: '1px solid rgba(255,255,255,0.10)' }}>
                            ...
                            <div className="space-y-3 max-h-64 overflow-y-auto scrollbar-thin scrollbar-track-transparent scrollbar-thumb-teal">
                                {friends.map((friend) => (
                                    <div key={friend.id} className="flex items-center justify-between text-sm text-grayblue">
                                        <div className="flex items-center space-x-3">
                                            <div className={`w-3 h-3 rounded-full ${friend.status === 'online' ? 'bg-green-400' : friend.status === 'in-game' ? 'bg-yellow-400' : 'bg-gray-500'}`}></div>
