...
const LiveLeaderboard: React.FC<Props> = ({ players, myUserId, maxEntries = 3 }) => {
  // Sort players by score descending
  const sortedPlayers = [...players].sort((a, b) => b.score - a.score);

  // Limit to max entries
  const visiblePlayers = maxEntries ? sortedPlayers.slice(0, maxEntries) : sortedPlayers;
  ...
  return (
    ...
    >
      ...
      <div className="mb-2.5 flex items-center gap-1.5">
        ...
        <h3 className="text-sm font-bold text-[#E6F6FA]">Leaderboard</h3>
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between px-2.5 py-1.5">
                ...
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      <span
                        className={`font-semibold truncate text-xs ${isMe ? 'text-[#E6F6FA]' : 'text-white'}`}
                      >
                        {player.name}
                      </span>
                      {isMe && (
                ...
                {/* Right: Score */}
                <div className="flex-shrink-0 font-bold text-xs text-teal">{player.score}</div>
              </div>
            </motion.div>
          );
        })}
