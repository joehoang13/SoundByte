...
function sign(user) {
    return jwt.sign({ sub: String(user._id), email: user.email }, JWT_SECRET, { expiresIn: JWT_EXPIRES_IN });
}
...
exports.signup = async (req, res) => {
  try {
    ...
    res.status(201).json({ token, user: { id: String(doc._id), email: doc.email, username: doc.username } });
  } catch (e) {
    ...
    return res.status(500).json({ error: 'Signup failed', ...(process.env.NODE_ENV !== 'production' ? { message: e.message } : {}) });
  }
};
